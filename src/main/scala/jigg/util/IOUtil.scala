...
object IOUtil {
  ...
  def openOut(file: File): BufferedWriter = bufWriter(outStream(file))
  ...
  def outStream(file: File) = file match {
    case gzipped if gzipped.getPath.endsWith(".gz") => new GZIPOutputStream(new FileOutputStream(gzipped))
    case file => new FileOutputStream(file)
  }
  ...
  def withTemp[A](prefix: String, suffix: String, delete: Boolean = true)(f: File=>A): A = {
    val file = File.createTempFile(prefix, suffix)

    if (delete) try f(file) finally file.delete
    else f(file)
  }
  def withOpen(path: String)(f: BufferedWriter=>Unit): Unit = {
    val writer = openOut(path)
    f(writer)
    writer.flush
    writer.close
  }
